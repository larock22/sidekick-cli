"""
Module: tinyagent.services.undo_service

Provides Git-based undo functionality for TinyAgent operations.
...
from tinyagent.constants import (ERROR_UNDO_INIT, UNDO_DISABLED_HOME, UNDO_DISABLED_NO_GIT,
                                UNDO_GIT_TIMEOUT, UNDO_INITIAL_COMMIT)
from tinyagent.core.state import StateManager
from tinyagent.exceptions import GitOperationError
from tinyagent.ui import console as ui
from tinyagent.utils.system import get_session_dir
...
def init_undo_system(state_manager: StateManager) -> bool:
    ...
    in the ~/.tinyagent/sessions/<session-id> directory.
    ...
    tinyagent_git_dir = session_dir / ".git"

    # Check if already initialized
    if tinyagent_git_dir.exists():
        return True
    ...
    try:
        ...
        git_dir_arg = f"--git-dir={tinyagent_git_dir}"
...
def commit_for_undo(
    message_prefix: str = "tinyagent", state_manager: Optional[StateManager] = None
) -> bool:
    """
    ...
    if not tinyagent_git_dir.exists():
        return False
